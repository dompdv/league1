...
def split_teams_into_groups(matches, teams):
    ...
    goal_marked = {t:0 for t in teams}
    goal_received = {t:0 for t in teams}
    for r in matches:
    ...
    group_size_old = len(goal_marked_ordered) / 4
    ...
    total_match = sum(len(played[t]) for t in teams)
    group_size = total_match / 4
    ...
    attack_group = {t: min(g, 3) for (t,_), g in zip(goal_marked_ordered, goal_marked_match)}
    #print(attack_group)
    ...
    defense_group = {t: min(g, 3) for (t,_), g in zip(goal_received_ordered, goal_received_match)}
    #print(defense_group)
    return attack_group, defense_group

def compute_base_statistics(matches, attack_group, defense_group, min_score, max_score):
...
def print_base_statistics_array(stats, min_score, max_score):
    ra = range(4)
    sca = range(min_score, max_score + 1)
    data_length = []
    print("Stats")
...
def print_base_statistics_vector(stats, min_score, max_score):
    ra = range(4)
    sca = range(min_score, max_score + 1)
    print("Stats vector")
...
def build_vectors(base_statistics, base_2, min_score, max_score):
    vectors = {}
    ra = range(4)
    sca = range(min_score, max_score + 1)
    for Aa, Ad, Ba, Bd in itertools.product(ra, ra, ra, ra):
...
def print_vectors(vectors, min_score, max_score):
    ra = range(4)
    sca = range(min_score, max_score + 1)
    print("Stats vector")
    print("Aa,Ad,Ba,Bd,", end='')
...
def compute_rebuilt_matrices(from_year, to_year, proba_table_file, threshold_1, threshold_2, printing=True):
    ...
    attack_group, defense_group = split_teams_into_groups(matches, teams)

    # Première approche : cumuler par Aa,Ad, Ba,Bd,s1,s2
    base_statistics, base_2 = compute_base_statistics(matches, attack_group, defense_group, min_score, max_score)
    #print_base_statistics_vector(base_statistics, min_score, max_score)

    # Construire des vecteurs (de probabilité de mettre des buts/ probabilités d'en encaisser
    vectors = build_vectors(base_statistics, base_2, min_score, max_score)
    vectors_rebuilt = build_vectors_rebuilt(vectors, threshold_1, threshold_2)
    #print_vectors(vectors_rebuilt, min_score, max_score)
...
if __name__ == "__main__":
    compute_rebuilt_matrices(2011, 2018, 'data_built_m.csv', threshold_1=1, threshold_2=1, printing=True)