...
class ModelAttackDefense:
    def __init__(self, n_teams, n_levels=4, max_goals=7, options=None):
        ...
        self.proba_table = proba_table.proba_table()
        if options and 'proba_table_file' in options:
            self.proba_table2 = proba_table.proba_table2(options['proba_table_file'])
        else:
            self.proba_table2 = proba_table.proba_table2()
    ...
    def account_for(self, team_1, team_2, score_1, score_2, home='N'):
        def reverse_home(h):
            if h is 'N':
                return 'N'
            return 'H' if h is 'E' else 'E'

        # team_1 attack vs team_2 defense
        p_team_1 = self.attack_vector[team_1]
        p_team_1.shape = (len(p_team_1), 1)
        p_team_2 = self.defense_vector[team_2]
        probabilities = self.proba_table[home][:, :, score_1] * p_team_1 * p_team_2
        # normalize
        s = np.sum(probabilities)
        probabilities = probabilities / s
        # accumulate attack_vector per user
        self.attack_vector[team_1] = np.sum(probabilities, axis=1)
        self.defense_vector[team_2] = np.sum(probabilities, axis=0)

        # team_1 defense vs team_2 attack
        p_team_1 = self.attack_vector[team_2]
        p_team_1.shape = (len(p_team_1), 1)
        p_team_2 = self.defense_vector[team_1]
        probabilities = self.proba_table[reverse_home(home)][:, :, score_2] * p_team_1 * p_team_2
        # normalize
        s = np.sum(probabilities)
        probabilities = probabilities / s
        # accumulate attack_vector per user
        self.attack_vector[team_2] = np.sum(probabilities, axis=1)
        self.defense_vector[team_1] = np.sum(probabilities, axis=0)
